...
prepare_extra_common() {
    ...
    echo "libunibreak${TARGET_DIR}/lib/libunibreak.so* var/packages/Jellyfin/target/lib" >> ${DPKG_INSTALL_LIST}
    popd
    popd
...
prepare_extra_arm() {
    ...
    echo "rkmpp${TARGET_DIR}/lib/librockchip_mpp.so* var/packages/Jellyfin/target/lib" >> ${DPKG_INSTALL_LIST}
    popd
    popd
    popd
